...
const DB = {
  ...
  async createUser(user) {
    ...
    for (const b of user.brands) {
      await supabaseClient.from('brands').insert({id: b.id, user_id: user.id, name: b.name});
    }
  },

  async updateActiveBrand(userId, brandId) {
    await supabaseClient.from('users').update({active_brand_id: brandId}).eq('id', userId);
  },

  // 브랜드 관련
  async addBrand(userId, brand) {
    await supabaseClient.from('brands').insert({id: brand.id, user_id: userId, name: brand.name});
  },

  async deleteBrand(brandId) {
    await supabaseClient.from('orders').delete().eq('brand_id', brandId);
    await supabaseClient.from('brands').delete().eq('id', brandId);
  },

  // 주문 관련
  async getOrders(brandId) {
    const {data} = await supabaseClient.from('orders').select('*').eq('brand_id', brandId);
  ...
  async upsertOrders(brandId, orders) {
    ...
    for (let i = 0; i < rows.length; i += batchSize) {
      ...
      await supabaseClient.from('orders').upsert(batch, {onConflict: 'brand_id,order_no'});
    }
  },
